...
The rule table is the central decision point — every context-dependent
behaviour lives here rather than scattered across the main loop. Rules can
be overridden at runtime from config.yaml without touching code.
"""
...
def register_rule(context: str, gesture: str, intent_name: str, action: str | None, label: str) -> None:
    """Add or update a rule at runtime (used by load_rules_from_config)."""
    RULES.setdefault(context, {})[gesture] = (intent_name, action, label)


def load_rules_from_config(config: dict) -> int:
    """Merge config.yaml's context_rules into RULES. Returns count loaded."""
    import logging
...
def load_combo_rules_from_config(config: dict) -> int:
    """Merge config.yaml's combo_rules into COMBO_RULES. Returns count loaded."""
    import logging
    log = logging.getLogger(__name__)
...
def classify_combo(second_hand_present: bool, primary_gesture: str, context: str) -> Intent | None:
    """
    Return a combo Intent if a two-hand rule applies, otherwise None.
    Callers should fall through to classify() when this returns None.
    """
    if not second_hand_present:
        return None
    entry = COMBO_RULES.get(context, {}).get(primary_gesture) or COMBO_RULES.get(DEFAULT_CONTEXT, {}).get(primary_gesture)
    if not entry:
        return None

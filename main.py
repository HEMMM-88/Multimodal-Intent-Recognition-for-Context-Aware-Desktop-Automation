...
""")
    try:
        ...
        return
    print("Context overrides (from config.yaml):")
...
def run(config_path: str, no_overlay: bool = False, skip_validation: bool = False):
    ...
    gesture_log_enabled = bool(log_cfg.get("gesture_csv_enabled", True))
    gesture_logger      = GestureLogger(
        log_dir=Path(__file__).resolve().parent / "logs",
        enabled=gesture_log_enabled,
    )
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                if len(all_lms) == 2:
                    ...
                    label = handedness_list[0].classification[0].label if handedness_list else "Right"
                    if swap_handedness:
                ...
                if not paused:
                    ...
                    if gesture == "open_palm" and pointer_norm and not two_hand_open_palm_active:
                        ...
                        if horiz_intent in HORIZONTAL_MOTION_ACTIONS:
                            fwd_a, back_a, fwd_lbl, back_lbl = HORIZONTAL_MOTION_ACTIONS[horiz_intent]
                            if open_palm_prev_x is not None:
                                ...
                                if abs(dx) >= video_seek_threshold and _debounced("horizontal_motion", debounce_clock, video_seek_debounce, now):
                                    execute_action(fwd_a if dx > 0 else back_a)
                                    display_action = fwd_lbl if dx > 0 else back_lbl
                    ...
                    if (two_hand_enabled and swipe_gate_open
                        ...
                        if both_swipe_prev_x is not None:
                            dx_primary  = palm_center[0] - both_swipe_prev_x[0]
                            dx_modifier = modifier_palm_x - both_swipe_prev_x[1]
                            if (abs(dx_primary) >= two_hand_swipe_threshold
                                    and abs(dx_modifier) >= two_hand_swipe_threshold
                                    and (dx_primary > 0) == (dx_modifier > 0)):
                                dir_key = "two_hand_swipe_right" if dx_primary > 0 else "two_hand_swipe_left"
                                if _debounced(dir_key, debounce_clock, two_hand_swipe_debounce, now):
                                    swipe_intent = classify_intent("two_hand_swipe", app_context)
                                    if swipe_intent.name in TWO_HAND_SWIPE_ACTIONS:
                                        fwd, back, fwd_lbl, back_lbl = TWO_HAND_SWIPE_ACTIONS[swipe_intent.name]
                                        execute_action(fwd if dx_primary > 0 else back)
                                        display_action = fwd_lbl if dx_primary > 0 else back_lbl
                    ...
                    if (two_hand_enabled and swipe_gate_open
                        ...
                        if both_swipe_prev_y is not None:
                            dy_primary  = palm_center[1] - both_swipe_prev_y[0]
                            dy_modifier = modifier_palm_y - both_swipe_prev_y[1]
                            horiz_dx    = abs(palm_center[0] - both_swipe_prev_x[0]) if both_swipe_prev_x else 0.0
                            if (abs(dy_primary) >= two_hand_swipe_vertical_threshold
                                    and abs(dy_modifier) >= two_hand_swipe_vertical_threshold
                                    and (dy_primary > 0) == (dy_modifier > 0)
                                    and abs(dy_primary) > horiz_dx * 1.2):
                                vdir_key = "two_hand_swipe_v_up" if dy_primary < 0 else "two_hand_swipe_v_down"
                                if _debounced(vdir_key, debounce_clock, two_hand_swipe_vertical_debounce, now):
                    ...
                    if gesture == "two_finger_tap":
                        ...
                        if (not two_finger_fired
                            ...
                            target_intent = combo_intent or classify_intent("two_finger_tap", app_context)
                            execute_action(target_intent.action)
                            display_action   = target_intent.label
                    ...
                    if gesture == "thumbs_up":
                        if combo_intent is not None:
                            if _debounced("thumbs_up_combo", debounce_clock, combo_debounce_seconds, now):
                                execute_action(combo_intent.action)
                                display_action = combo_intent.label
                        ...
                        if combo_intent is not None:
                            if _debounced("thumbs_down_combo", debounce_clock, combo_debounce_seconds, now):
                                execute_action(combo_intent.action)
                                display_action = combo_intent.label
...
def main():
    ...
    parser.add_argument("--list-gestures",   action="store_true",   help="Print gesture reference and exit")
    parser.add_argument("--skip-validation", action="store_true",   help="Skip config.yaml validation")
